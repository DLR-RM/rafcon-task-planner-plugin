...
class StateMachineGenerator:
    ...
    def generate_state_machine(self):
        ...
        if is_independent_sm:
            ...
            import copy
            n_target_state_m = copy.deepcopy(target_state_m)
            root_state, state_order_list = self.__generate_core_machine(sm_name, n_target_state_m.state)

            if root_state:
                #suppress gui
                call_gui_callback(substitute_state, target_state_m, n_target_state_m)
                wait_for_gui()
                ...
                logger.info(sm_name + " contains " + str(len(root_state.states)) + " states.")
                logger.info("State machine generation took {0:.4f} seconds.".format(time.time() - start_time))
                n_target_state_m.action_signal.emit(ActionSignalMsg(action='substitute_state', origin='model',
                                                                  action_parent_m=n_target_state_m,
                                                                  affected_models=[n_target_state_m], after=False))
                ...
                n_target_state_m.action_signal.emit(ActionSignalMsg(action='substitute_state', origin='model',
                                                                action_parent_m=n_target_state_m,
                                                                affected_models=[n_target_state_m], after=True))
